...
const T = {
  es: {
    ...
    ad_ldap_enum_obj_desc: 'Con la cuenta de servicio svc.readonly obtenida en AD 01, enumera los usuarios del dominio (la enumeración anónima no funciona contra este DC) y vuelca los objetos del directorio por LDAP. El servidor permite binds simples sin cifrado y un administrador dejó escrita una contraseña —y la flag— en el atributo description de un usuario.',
  ...
  en: {
    ...
    ad_ldap_enum_obj_desc: 'With the svc.readonly credential from AD 01, enumerate the domain users (anonymous enumeration does not work against this DC) and dump the directory objects over LDAP. The server allows simple binds without encryption and an administrator left a password — and the flag — written in a user description attribute.',
